...
const RadioSm: React.FC<RadioProps> = ({
  ...
  return (
    <label
      htmlFor={id}
      className={`flex cursor-pointer select-none items-center text-sm text-gray-500 text-gray-400 ${className}`}
    >
        ...
        <span
          className={`mr-2 flex h-4 w-4 items-center justify-center rounded-full border ${
            checked
              ...
              : "bg-transparent border-gray-300 border-gray-700"
          }`}
        >
          {/* Inner Dot */}
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              checked ? "bg-white" : "bg-white bg-[#1e2636]"
            }`}
          ></span>
        </span>
      </span>
