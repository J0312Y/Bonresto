...
class Api_v1_model extends CI_Model
{
	...
	public function foodlist($CategoryID = null)
	{
		...
		if (!empty($itemlist)) {
			...
			foreach ($itemlist as $items) {
				...
				$taxData = [];
				$totalTax = 0;
				if (count($taxSettings) <= 0 && $items->productvat <= 0) {
					$totalTax = $appSetting->vat;
				} else if (count($taxSettings) <= 0 && $items->productvat > 0) {
					$totalTax = $items->productvat;
				}
				...
				foreach ($taxSettings as $index => $tax) {
					...
					if (isset($items->$productListIndex) && $items->$productListIndex) {
						$taxElement["value"] = $items->$productListIndex;
					} else {
				...
				$output[$k]['taxInfo'] = $taxData;
				$output[$k]['taxTotal'] = $totalTax;
	...
	{
		$arrayCategoryID = explode(',', $CategoryID);
		$weherein = 'item_foods.CategoryID IN(' . $CategoryID . ')';
		$this->db->select('*');
		$this->db->from('item_foods');
		$this->db->where('ProductsIsActive', 1);
		$this->db->where_in('CategoryID', $arrayCategoryID);
		$weherein = 'item_foods.CategoryID IN(' . $CategoryID . ')';
		$query = $this->db->get();
		$itemlist = $query->result();
	...
	public function taxchecking()
	{
		...
		if (!empty($taxsetting)) {
			if ($taxsetting->tax == 1) {
				$taxinfos = $this->db->select('*')->from('tax_settings')->get()->result();
			}
		}
